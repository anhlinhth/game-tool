...
// require_once ROOT_APPLICATION.DS.'modules'.DS.'localite'.DS.'models'.DS.'Language.php';
// require_once ROOT_APPLICATION.DS.'modules'.DS.'localite'.DS.'models'.DS.'Group.php';

class Localize_LGroupController extends Base_Controller
{
public function _setUserPrivileges()
	...
	public function indexAction(){	
		$model = new Localize_Model_Group();		
		...
		if ($this->_request->isPost ()) {
			...
			if($this->_request->getParam("btsave")){
				foreach ( $model->GetAllGroup() as $value) {
					if($this->_request->getParam($value['id'])){
						$this->_redirect('/localite/lgroup/ledit?id='.$value['id']);
					}
				}
			}
		}	
		...
		if ($this->_request->isPost ()) {
			...
			if($name){
				if($model->CheckExistGroup($name) == false){
					...
					$this->_redirect('/localite/lgroup');
				}	
	...
	public function ldeleteAction(){	
		...
		if($id){
			...
			else $this->_redirect('/localite/lgroup?msg=Nhóm không tồn tại');	
		}
		else 
			$this->_redirect('/localite/lgroup?msg=Nhóm không tồn tại');	
	}
